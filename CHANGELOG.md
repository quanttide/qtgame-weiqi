...
### Added
- 加 Pass 按钮和 AI 自动 Pass 阈值，终局可达
- 加胜负判定：连 Pass 终局 + 中国规则数子
- 实时比分 + 独立 score.js（含死子判定）
- AI 用计分器判断是否还有活棋，被全包围时主动 Pass

### Changed
- 职责分离：checkAIMove 迁入 ai.js，main.js 保留主干
- 收紧 AI Pass 阈值：避免开局误判

### Fixed
- 修复 AI 开局因活棋=0 直接 Pass 的 bug
